...
void CSocket ::Allocate(int type)
{
	...
	if (m_Socket == INVALID_SOCKET) {
		...
		Log::console("[SOCKET] error (socket) - " + GetErrorString());
		return;
	}
}
...
void CTCPSocket ::DoRecv(fd_set* fd)
{
	...
	if (FD_ISSET(m_Socket, fd)) {
		...
		if (c == SOCKET_ERROR && GetLastError() != EWOULDBLOCK) {
			...
			m_HasError = true;
			m_Error = GetLastError();
			Log::console("[TCPSOCKET] error (recv) - " + GetErrorString());
			return;
		}
		else if (c == 0) {
			// the other end closed the connection

			Log::console("[TCPSOCKET] closed by remote host");
			m_Connected = false;
		}
		else if (c > 0) {
...
void CTCPSocket ::DoSend(fd_set* send_fd)
{
	...
	if (FD_ISSET(m_Socket, send_fd)) {
		...
		if (s == SOCKET_ERROR && GetLastError() != EWOULDBLOCK) {
			...
			m_HasError = true;
			m_Error = GetLastError();
			Log::console("[TCPSOCKET] error (send) - " + GetErrorString());
			return;
		}
		else if (s > 0) {
...
void CTCPClient ::Connect(string localaddress, string address, uint16_t port)
{
	...
	if (!localaddress.empty()) {
		...
		if (::bind(m_Socket, (struct sockaddr*)&LocalSIN, sizeof(LocalSIN)) == SOCKET_ERROR) {
			m_HasError = true;
			m_Error = GetLastError();
			Log::console("[TCPCLIENT] error (bind) - " + GetErrorString());
			return;
		}
	}
	...
	if (!HostInfo) {
		m_HasError = true;
		// m_Error = h_error;
		Log::console("[TCPCLIENT] error (gethostbyname)");
		return;
	}
	...
	if (connect(m_Socket, (struct sockaddr*)&m_SIN, sizeof(m_SIN)) == SOCKET_ERROR) {
		if (GetLastError() != EINPROGRESS && GetLastError() != EWOULDBLOCK) {
			...
			m_HasError = true;
			m_Error = GetLastError();
			Log::console("[TCPCLIENT] error (connect) - " + GetErrorString());
			return;
		}
	}
...
bool CTCPServer ::Listen(string address, uint16_t port)
{
	...
	if (::bind(m_Socket, (struct sockaddr*)&m_SIN, sizeof(m_SIN)) == SOCKET_ERROR) {
		m_HasError = true;
		m_Error = GetLastError();
		Log::console("[TCPSERVER] error (bind) - " + GetErrorString());
		return false;
	}
	...
	if (listen(m_Socket, 8) == SOCKET_ERROR) {
		m_HasError = true;
		m_Error = GetLastError();
		Log::console("[TCPSERVER] error (listen) - " + GetErrorString());
		return false;
	}
...
bool CUDPSocket ::SendTo(string address, uint16_t port, string message)
{
	if (m_Socket == INVALID_SOCKET || m_HasError)
		return false;
	...
	if (!HostInfo) {
		m_HasError = true;
		// m_Error = h_error;
		Log::console("[UDPSOCKET] error (gethostbyname)");
		return false;
	}
...
bool CUDPSocket ::Broadcast(uint16_t port, BYTEARRAY message)
{
	...
	if (sendto(m_Socket, MessageString.c_str(), MessageString.size(), 0, (struct sockaddr*)&sin, sizeof(sin)) == -1) {
		Log::console("[UDPSOCKET] failed to broadcast packet (port " + UTIL_ToString(port) + ", size " + UTIL_ToString(MessageString.size()) + " bytes)");
		return false;
	}
...
{
	if (subnet.empty()) {
		Log::console("[UDPSOCKET] using default broadcast target");
		m_BroadcastTarget.s_addr = INADDR_BROADCAST;
	}
	else {
		...
		Log::console("[UDPSOCKET] using broadcast target [" + subnet + "]");
		...
		if (m_BroadcastTarget.s_addr == INADDR_NONE) {
			Log::console("[UDPSOCKET] invalid broadcast target, using default broadcast target");
			m_BroadcastTarget.s_addr = INADDR_BROADCAST;
		}
	}
}
...
bool CUDPServer ::Bind(struct sockaddr_in sin)
{
	...
	if (::bind(m_Socket, (struct sockaddr*)&m_SIN, sizeof(m_SIN)) == SOCKET_ERROR) {
		m_HasError = true;
		m_Error = GetLastError();
		Log::console("[UDPSERVER] error (bind) - " + GetErrorString());
		return false;
	}
...
void CUDPServer ::RecvFrom(fd_set* fd, struct sockaddr_in* sin, string* message)
{
	...
	if (FD_ISSET(m_Socket, fd)) {
		...
		if (c == SOCKET_ERROR && GetLastError() != EWOULDBLOCK) {
			...
			m_HasError = true;
			m_Error = GetLastError();
			Log::console("[UDPSERVER] error (recvfrom) - " + GetErrorString());
		}
		else if (c > 0) {
			// success!
